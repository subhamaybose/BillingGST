<?php
	...
	if(isset($_POST['purchase_return_no']))
	{
		//echo "In here cos : " . mysql_num_rows($tok_chk_old);
		//The main SQL
		
		$sql_main = "insert into purchase_return set purchase_return_no = '" . $_POST['purchase_return_no'] . "', purchase_return_date = '" . date('Y-m-d', strtotime($_POST['purchase_return_date'])) . "', purchase_return_amount = '" . $_POST['purchase_return_amount'] . "', party_id = '" . $_POST['party_id'] . "', invoice_no = '" . trim($_POST['invoice_no']) . "', purchase_order_id = '" . trim($_POST['purchase_order_id']) . "'";
					
		$tok_main = mysql_query($sql_main);
		
		$purchase_return_id = mysql_insert_id();
		...
		foreach($_POST['item_id'] as $item_key => $item_val)
		{
			if(trim($item_val) != '')
			{
				$sql_sub = "insert into purchase_return_details set purchase_return_id = '" . $purchase_return_id . "', item_id = '" . $item_val . "', item_qty = '" . $_POST['item_qty'][$item_key] . "', item_unit = '" . $_POST['item_unit'][$item_key] . "', item_rate = '" . $_POST['item_rate'][$item_key] . "', item_amount = '" . $_POST['item_amount'][$item_key] . "'";
		...
		$dr_cr='D';
	$nara="Being Debit Note No :".$_POST['purchase_return_no'] ;
	post_ledger($_POST['purchase_return_no'],$_POST['purchase_return_amount'],$dr_cr,$_POST['party_id'],date('Y-m-d', strtotime($_POST['purchase_return_date'])),$nara);
		//Once the save is done, return to the listing page
		header("location: ./purchase_return_list.php?msg=" . $msg);
	}
...
<link rel="stylesheet" type="text/css" href="calender/dhtmlgoodies_calendar.css" />
	...
	<?php
		...
		while($data_po = mysql_fetch_assoc($tok_po))
		{
			...
			po_party_rel_arr[<?php echo $arr_ctr ?>][0] = "<?php echo $data_po['purchase_bill_no']; //The purchase bill no ?>";
	...
	function calculatePurchaseTotals()
	{
		...
		document.getElementById("purchase_return_amount").value = gross_amnt;
	}
	
	function IsNill(frm)
	{
	
		   if(frm.purchase_order_id.value.length=="")
		   {
				   alert("Please enter Purchase order id");
				   frm.purchase_order_id.focus();
				   return false;        
		   }
		   
		   if(frm.purchase_return_date.value.length=="")
		   {
				   alert("Please enter purchase return date");
				   frm.purchase_return_date.focus();
				   return false;        
		   }
...
var w=window.open(ff,'mywindow',"width=1200, height=1000, top=10, left=10");
		...
		function open_sale()
		{
			if(document.getElementById("purchase_order_id").selectedIndex!=0)
			{
				selval=document.getElementById("purchase_order_id").options[document.getElementById("purchase_order_id").selectedIndex].value;
				openpop("./purchase_view.php?bill_no="+selval);
				//purchase_order_id
			}else{
				alert("Please select a purchase bill to view");
			}
		}
	</script>
</head>
                ...
                <tr>
				  ...
				  <input name="item_id" type="hidden" value="<?php echo $_REQUEST["item_id"] ?>" />
                   ...
                   <table width="100%" border="0" cellspacing="0" cellpadding="5">
                    <tr>
                        <td height="30" colspan="3" background="images/login_bg.gif"><strong> Purchase Return Bill </strong></td>
                      </tr>
                      <tr>
                    ...
                    <tr>
                      <td>
                        <label>Purchase Return No .  (*): </label></td>
                      <td height="20"><input name="purchase_return_no" class="input" id="purchase_return_no" value="" tabindex="3" size="20" /></td>
                    </tr>
					<tr>
                      <td>
                        <label>Invoice No .  (*): </label></td>
						  <td height="20">
						  	<select name="purchase_order_id" id="purchase_order_id" onchange="set_party(this.value)">
								...
								{
									?>
									<option value="<?php echo $data_po['purchase_bill_no'] ?>"><?php echo $data_po['purchase_bill_no'] ?></option>
									<?php
								}
								?>
					...
					<tr>
                      <td>Return Date (*):</td>
                      <td valign="middle"><input name="purchase_return_date" class="input" id="purchase_return_date" tabindex="2"  size="20" readonly />&nbsp;
					 ...
					 <img src="calender/btn_dropdown.gif" alt="Select Date" width="18" height="18" onClick="displayCalendar(document.getElementById('purchase_return_date'),'mm/dd/yyyy',this); return false;" align="absbottom" /></td>
                    </tr>
   					<tr>
                      <td>Party Name. (*):</td>
                      <td height="20">
					...
					<tr>
                      <td>Total Amount. :</td>
                      <td height="20">
					  	<input name="purchase_return_amount" class="input" id="purchase_return_amount" tabindex="10" value="0" readonly onkeypress="return isNumberKey(event)" size="20" />
					  </td>
                    </tr>
                   	<tr>
                      <td>&nbsp;</td>
