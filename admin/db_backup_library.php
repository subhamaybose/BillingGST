...
*/	
	...
	class db_backup{
		...
		public function download($name='backup'){
			...
			$name = "asmi_backup_".date("Y_m_d_h_i_s_a");
		...
		public function save($path,$name=""){
			$name = ($name != "") ? $name : 'asmi_' . date('Y-m-d-h-i-s-a');
			...
			if(!$fw){
				...
				$sql = "INSERT INTO `backup`(`backup_id`, `backup_file`, `backup_date`) VALUES ('','".$name."','".time()."')";
				mysql_query($sql);
				$qlf =mysql_fetch_assoc(mysql_query( "SELECT * FROM `backup` WHERE `backup_id`= (SELECT * FROM (SELECT `backup_id` FROM `backup` ORDER BY `backup_id` DESC LIMIT 1 OFFSET 10) AS p)"));
				if($qlf["backup_file"]!=''){
					@unlink("./backup/".$qlf["backup_file"].".sql");
				}
				$sql_del=mysql_query("DELETE FROM `backup` WHERE `backup_id`= (SELECT * FROM (SELECT `backup_id` FROM `backup` ORDER BY `backup_id` DESC LIMIT 1 OFFSET 10) AS p)");
				return true; 
			}
		}
