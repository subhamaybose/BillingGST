...
{
if($prn==0)
$strout= '<tr  height="430">';
else
$strout= '<tr  height="auto">';
			$strout.='<td>
...
<table height="920" width="690" border="1" cellpadding="0" cellspacing="0"  >
 ...
 <?php
	...
	$cnt=1;
					while($data_item = mysql_fetch_assoc($tok_item))
					{
						...
						$itemDet[$cnt]["item_name"]=$data_details['item_name'];
						$itemDet[$cnt]["item_decription"]=$data_details['item_decription'];
						$itemDet[$cnt]["category"]=$data_item['category'];
						$itemDet[$cnt]["capacity"]=$data_item['capacity'];
						$itemDet[$cnt]["unit_name"]=$data_unit['unit_name'];
						$itemDet[$cnt]["item_qty"]=$data_item['item_qty'];
						$itemDet[$cnt]["item_rate"]=$data_item['item_rate'];
						$itemDet[$cnt]["item_amount"]=$data_item['item_amount'];
						$cnt++;
					}
						?>
 ...
 <?php
 echo "<tr><td height=\"150\" valign='top'><table border='0' width='100%'>";
 foreach($itemDet as $key => $val)
 {
 
 ?>
 						<tr height="33">
							<td><?php echo $key ?></td>
							<td><?php echo $val['item_name'] . "<br /><i>" . $val['item_decription'] . '</i>' ?></td>
							<td align="center"><?php echo $val['category'] ?>&nbsp;</td>
							<td align="right"><?php echo $val['capacity'] ?>&nbsp;</td>
 							<td align="right"><?php echo $val['item_qty'] . ' ' . $val['unit_name']; ?></td>
							<td align="right"><?php echo $val['item_rate'] ?></td>
							<td align="right"><?php echo $val['item_amount'] ?></td>
						</tr>
 <?php
 	
	 if($key%$printPerPage==0) 
	 {
	 if($tot_row>$key)
		{
		echo "</table></td></tr>";
		$page_count=($key/$printPerPage)+1;
		?>
		<?php bill_footer(0);
		echo "</table><table height=\"920\" width=\"690\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"page-break-before:always;\" >";
		 bill_header($data_sale,$page_count); ?>
		<?php sub_head(); ?> 
		<?php
		 echo "<tr><td height=\"150\"><table border='0' width='100%'>";
		 }
 	 }
	 if($key==$tot_row)
	 {
