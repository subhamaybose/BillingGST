...
if($mode=="del"){
	$sql_delete="DELETE FROM purchase_return WHERE purchase_return_id = '" . trim($_REQUEST['purchase_return_id']). "'";
	//echo $sql_delete;
	mysql_query($sql_delete);
	
	//and the relations
	$sql_delete="DELETE FROM purchase_return_details WHERE purchase_return_id = '".trim($_REQUEST['purchase_return_id'])."'";
...
function delChk(purchase_return_id){
	if(confirm("Do you want to delete the Purchase Return Bill ? ")){
	 window.location.href="./purchase_return_list.php?purchase_return_id="+purchase_return_id+"&mode=del";
	}
}
             ...
             <table width="99%" border="0" cellpadding="0" cellspacing="1" bgcolor="#999999">
                ...
                <tr>
                  <td bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="2" cellpadding="2">
                      <tr>
                        <td height="30" colspan="4" background="images/login_bg.gif"><img src="images/login_bg.gif" width="15" height="30" align="absmiddle" /><strong>Purchase Return List</strong></td>
						<td align="right"  background="images/login_bg.gif"><input name="adbutton" type="button" class="btn" value="Add" onclick="JavaScript:window.location.href='purchase_return_master.php?mode=add&<?=$qstring?>'" /></td>
                      </tr>
                      ...
                      <tr>
                        <td width="15%" height="30" align="center" bgcolor="#e6e6e6"><strong>Purchase Return No</strong></td>
					  ...
					  $sql_purchases="select count(*) from purchase_return";
					  ...
					  $sql_purchases="select *  from purchase_return order by purchase_return_date";
					  ...
					  while($row_purchases=mysql_fetch_array($rs_purchases)){
						...
						$data_cust = mysql_fetch_assoc($tok_cust);
					  	?>
						  <tr>
							<td width="10%" align="center"><?php echo $row_purchases['purchase_return_no']; ?></td>
							<td width="10%" align="center"><?php echo date('d-m-Y', strtotime($row_purchases['purchase_return_date'])); ?></td>
							<td width="20%" height="30" align="center" nowrap="nowrap"><?php echo $row_purchases['purchase_return_amount']; ?></td>
							<td width="50%" align="center" nowrap="nowrap"><?php echo $data_cust['party_name']; ?></td>
 							<td align="center" nowrap="nowrap">
							 <input name="delbutton" type="button" class="btn" value="Delete" onclick="delChk('<?php echo $row_purchases['purchase_return_id']; ?>')" />
                    ...
                    <tr>
                      <?php
							if ($pageNumber > 1)  {
								?>
                      <td width="101" height="25" align="right" class="style1"><a href="purchase_return_list.php?page=1&max=1&<?=$qstring?>" class="lnk">&lt;&lt;</a></td>
                      <td width="66" align="right"><a href="purchase_return_list.php?page=<?php print($pageNumber - 1); ?>&max=1&<?=$qstring?>" class="lnk">&lt;</a></td>
                      <?php
							}
							else {
								?>
                      ...
                      <?php
							if ($pageNumber < $pages) {
								?>
                      <td width="80" height="25" align="left"><a href="purchase_return_list.php?page=<?php print($pageNumber + 1); ?>&max=1&<?=$qstring?>" class="lnk">&gt;</a></td>
                      <td width="46" align="left"><a href="purchase_return_list.php?page=<?php print($pages); ?>&max=1&<?=$qstring?>" class="lnk">&gt;&gt;</a></td>
                      <?php
							}
							else {
								?>
