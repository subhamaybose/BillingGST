...
<?
		if($_POST["party_id"]!="") {
			...
			$sel_cr="select sum(amount) from ledger b where dr_cr='C' and to_days('$finPrevEndDate')-to_days(led_date)>=0 and to_days(led_date)-to_days('$finPrevStartDate') >=0  $cond";
			...
			$sel_dr="select sum(amount) from ledger b where dr_cr='D' and to_days('$finPrevEndDate')-to_days(led_date)>=0 and to_days(led_date)-to_days('$finPrevStartDate') >=0  $cond";
...
<td colspan="3" width="50%" valign="top">
			...
			$sel_cr="select sum(amount) from ledger b where dr_cr='C' and to_days('$start_date')-to_days(led_date)>=0  and to_days(led_date)-to_days('$finPrevEndDate') >0 $cond";
			$sql_cr_list=mysql_query($sel_cr) or die(mysql_error());
			if($s_cr_list=mysql_fetch_array($sql_cr_list))
			{
			$prev_cr=$s_cr_list[0];
			...
			$sel_dr="select sum(amount) from ledger b where dr_cr='D' and to_days('$start_date')-to_days(led_date)>=0  and to_days(led_date)-to_days('$finPrevEndDate') >0 $cond";
			$sql_dr_list=mysql_query($sel_dr) or die(mysql_error());
			if($s_dr_list=mysql_fetch_array($sql_dr_list))
			{
			$prev_dr=$s_dr_list[0];
