...
function delChk(bill_no){
	if(confirm("Do you want to delete the Sale Bill ? ")){
	 window.location.href="./sales_list_vat.php?bill_no="+bill_no+"&mode=del&fin_year=<?=$fin_year?>";
	}
}
                ...
                <tr>
                  <td bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="2" cellpadding="2">
                      <tr>
						...
						<td align="right"  background="images/login_bg.gif"><input name="adbutton" type="button" class="btn" value="Add" onclick="JavaScript:window.location.href='sale_master.php?mode=add&<?=$qstring?>'"disabled /></td>
                      </tr>
					  ...
					  while($row_sales=mysql_fetch_array($rs_sales)){
						...
						$data_cust = mysql_fetch_assoc($tok_cust);
						  ...
						  <tr>
							...
							<td width="20%" height="30" align="center" nowrap="nowrap"><?php echo $row_sales['net_amount']; ?>
 							</td>
							<td align="center" nowrap="nowrap"> 
							   <input name="edbutton" type="button" class="btn" value="Re-Print" onclick="JavaScript:window.location.href='./sale_print_vat.php?bill_no=<?php echo $row_sales['bill_no']; ?>'" /> &nbsp;
							 <input name="delbutton" type="button" class="btn" value="Delete" onclick="delChk('<?php echo $row_sales['bill_no']; ?>')" disabled />
                    ...
                    <tr>
                      <?php
							if ($pageNumber > 1)  {
								?>
                      <td width="101" height="25" align="right" class="style1"><a href="sales_list_vat.php?page=1&max=1&<?=$qstring?>" class="lnk">&lt;&lt;</a></td>
                      <td width="66" align="right"><a href="sales_list_vat.php?page=<?php print($pageNumber - 1); ?>&max=1&<?=$qstring?>" class="lnk">&lt;</a></td>
                      <?php
							}
							else {
								?>
                      ...
                      <?php
							if ($pageNumber < $pages) {
								?>
                      <td width="80" height="25" align="left"><a href="sales_list_vat.php?page=<?php print($pageNumber + 1); ?>&max=1&<?=$qstring?>" class="lnk">&gt;</a></td>
                      <td width="46" align="left"><a href="sales_list_vat.php?page=<?php print($pages); ?>&max=1&<?=$qstring?>" class="lnk">&gt;&gt;</a></td>
                      <?php
							}
							else {
								?>
