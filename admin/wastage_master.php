...
if(isset($_POST['bsave'])){
	...
	if( $wastage_id==""){	
		...
		if(chkDuplicateUser(ucwords($_POST['wastage_no']))){
			...
			//echo $sql_insert;
				...
				$sql_stk = 'update item_master set item_stock = item_stock - ' .$_POST['wastage_qty'] .' where item_id = \'' . trim($_POST['item_id']) . '\'';
				...
				$_SESSION['err_msg']='Error : In Record already exists';
			
			}	
		}else{ 
			if(chkDuplicateUser(ucwords($_POST['wastage_no']),$_POST['wastage_id'])){
				
				$sql_edit="UPDATE wastage_master SET wastage_no='".trim(strtoupper($_POST['wastage_no']))."', ";
				$sql_edit.="'" . date('Y-m-d', strtotime($_POST['wastage_date'])) . "',item_type='".trim($_POST['item_type'])."',item_id='".trim($_POST['item_id'])."'";
				...
				$sql_stk = 'update item_master set item_stock = item_stock + ' .$_POST['old_wastage_qty'] .' where item_id = \'' . trim($_POST['item_id']) . '\'';
				$tok_stk = mysql_query($sql_stk);
				$sql_stk = 'update item_master set item_stock = item_stock - ' .$_POST['wastage_qty'] .' where item_id = \'' . trim($_POST['item_id']) . '\'';
				$tok_stk = mysql_query($sql_stk);
				...
				$_SESSION['err_msg']='Error : Ed Record already exists';
			}	
	}
 }
...
<link rel="stylesheet" type="text/css" href="calender/dhtmlgoodies_calendar.css" />
	...
	function IsNill(frm)
{
	   if(frm.wastage_no.value.length=="")
               {
                       alert("Please enter wastage no");
                       frm.wastage_no.focus();
                       return false;        
               }
	
       return true;
}
		function setItemCodePrefix(value)
		{
			document.frmWaste.mode.value='Reload';
			document.frmWaste.submit();
...
?>
              <table width="90%" border="0" cellpadding="0" cellspacing="1" bgcolor="#999999">
                <tr>
                  <td bgcolor="#FFFFFF">
				  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"  name="frmWaste" id="frmWaste" onsubmit="return IsNill(this)">
                   ...
                   <table width="100%" border="0" cellspacing="0" cellpadding="5">
                    <tr>
                      ...
                      <?php 
						...
						{
							$item_type=$r_edit["item_type"];
						}
					  ?>
					  	<select name="item_type" id="item_type" tabindex="3" onchange="setItemCodePrefix(this.value);">
                        <option value="" selected="selected">< -- Select -- ></option>
                        <option value="Raw Material" <?php if($item_type=="Raw Material") echo "selected"; ?>>Raw Material</option>
                        <option value="Work-In-Progress" <?php if($item_type=="Work-In-Progress") echo "selected"; ?>>Work-In-Progress</option>
						<option value="Finished Goods" <?php if($item_type=="Finished Goods") echo "selected"; ?>>Finished Goods</option>
                       </select>
					  </td>
                    </tr>
					...
					<tr>
					  ...
					  <?php
					  if($_POST["item_type"]!="")
					 	 $sql_imt="select * from item_master where item_type='".$item_type."'  order by item_name";
					  else
 					...
 					<tr>
                      <td>Quantity (*):</td>
                      <td height="20"><input name="wastage_qty" class="input" id="wastage_qty" tabindex="5" value="<?php echo $r_edit["wastage_qty"]?>" size="20" />
                      <input type="hidden" name="old_wastage_qty" class="input" id="old_wastage_qty" tabindex="5" value="<?php echo $r_edit["wastage_qty"]?>" size="20" />
					...
					<tr>
					 ...
					 <img src="calender/btn_dropdown.gif" alt="Select Date" width="18" height="18" onClick="displayCalendar(document.getElementById('wastage_date'),'mm/dd/yyyy',this); return false;" align="absbottom" />
                   	...
                   	<tr>
                      <td>&nbsp;</td>
                      <td><input type="submit" name="bsave" value="Save" /> &nbsp;<input type="button" value="Cancel" onclick="window.location.href='./wastage_list.php'"></td>
                    </tr>
                    <tr>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
